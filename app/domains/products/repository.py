...
from app.utils.classes.enums.unitenum import UnitEnum
...
class ProductRepository:
    ...
    async def update(self, product_id: int, name: str, unit: UnitEnum,
                     quantity: float, recommended: float) -> ProductOrm:
        stmt = (
            update(ProductOrm)
            .where(and_(ProductOrm.id == product_id))
            .values(name=name, unit=unit, quantity=quantity, recommended=recommended)
            .returning(ProductOrm)
        )
        result = await self.session.execute(stmt)
        return result.scalars().first()
