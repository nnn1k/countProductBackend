...
class UserInStorageRepository:
    ...
    async def get_one_storage_with_users(self, user_id: int, storage_id: int, rel: bool = False) -> StorageOrm:
        stmt = (
            select(StorageOrm)
            .join(UserInStorageOrm)
            .where(
    ...
    async def delete_user_in_storage(self, user_id: int, storage_id: int) -> bool:
        stmt = (
