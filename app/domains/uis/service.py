...
class UserInStorageService:
    ...
    async def get_one_storage_on_user(
        ...
        storage = await self.uis_repo.get_one_storage_with_users(user_id=user.id, storage_id=storage_id, rel=rel)
        if rel:
    ...
    async def delete_user(self, user: UserSchema, user_id: int, storage_id: int) -> None:
        if not await self.check_user_is_owner(user_id=user.id, storage_id=storage_id):
            raise user_is_not_owner_exc
        result = await self.uis_repo.delete_user_in_storage(user_id=user_id, storage_id=storage_id)
        if not result:
            raise user_in_storage_not_exist_exc

    async def check_user_is_owner(self, storage_id: int, user_id: int) -> bool:
        return await self.uis_repo.check_user_in_storage(storage_id=storage_id, user_id=user_id, is_owner=True)

    async def check_user_in_storage(self, storage_id: int, user_id: int) -> bool:
        return await self.uis_repo.check_user_in_storage(storage_id=storage_id, user_id=user_id)
